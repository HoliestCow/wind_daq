...
def tracker(graph, scfg, hp, run, design, frame_name_list, pos_x, pos_y, target_w, target_h, final_score_sz, filename, image, templates_z, scores, label, ownerProb, color, sFrame, eFrame, finalImages, sourceAngle, ):
    num_frames = np.size(frame_name_list)
    if(num_frames < eFrame):
        eFrame = num_frames
        ...
        for i in range(sFrame, eFrame):        
            ...
            image_, scores_ = sess.run(
                ...
                feed_dict={
                    scfg.pos_x_ph: pos_x,
                    scfg.pos_y_ph: pos_y,
                    scfg.x_sz0_ph: scaled_search_area[0],
                    scfg.x_sz1_ph: scaled_search_area[1],
                    scfg.x_sz2_ph: scaled_search_area[2],
            ...
            if run.visualization:
                if(newVector):
                    singleFrame = cv2.imread(frame_name_list[i],1)
                else:
                    singleFrame = finalImages[i-sFrame]
                cv2.rectangle(singleFrame,
                ...
                #elif(color == (0,0,255)):
                #    ownerProb = .32 / 2
                #cv2.putText(singleFrame,'P('+label+')='+str(ownerProb),(int(bboxes[i,0]),int(bboxes[i,1])),cv2.FONT_HERSHEY_SIMPLEX,0.5, color,2)
                if(newVector):
    ...
    #video.release()
    return bboxes, speed, finalImages
