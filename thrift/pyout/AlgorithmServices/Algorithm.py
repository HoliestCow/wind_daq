...
class Iface(object):
    def getAlgorithmDefinition(self):
        """
        Returns what the algorithm is capable of.
        """
        pass
    ...
    def definePTU(self, systemDefinition):
        ...
        Returns EITHER a list of components that the algorithm would like to subscribe to OR an empty list.
         ...
         -Algorithms MAY subscribe to other algorithms.  However, if two algorithms subscribe to each other, this
          results in a circular dependency and the resulting behavior undefined.  Please don't do this.
    ...
    def getAlgorithmConfiguration(self):
        """
        Gets how the algorithm is configured, such as background times, etc...
    ...
    def updateAlgorithm(self, systemData):
        ...
        The algorithm MUST respond with a systemData packet every 100 ms, however the system MAY choose to leave all optional
        fields blank (essentially returning no data) if the algorithm is not ready to return a result.
        ...
        while True:
            ...
            if fid == 0:
                if ftype == TType.STRUCT:
                    self.success = AlgorithmPayload.ttypes.AlgorithmDefinition()
...
getAlgorithmDefinition_result.thrift_spec = (
    (0, TType.STRUCT, 'success', [AlgorithmPayload.ttypes.AlgorithmDefinition, None], None, ),  # 0
)
...
class updateAlgorithm_result(object):
    ...
    def read(self, iprot):
        ...
        while True:
            ...
            if fid == 0:
                if ftype == TType.STRUCT:
                    self.success = AlgorithmPayload.ttypes.AlgorithmData()
                    self.success.read(iprot)
                else:
                    iprot.skip(ftype)
            else:
...
updateAlgorithm_result.thrift_spec = (
    (0, TType.STRUCT, 'success', [AlgorithmPayload.ttypes.AlgorithmData, None], None, ),  # 0
)
