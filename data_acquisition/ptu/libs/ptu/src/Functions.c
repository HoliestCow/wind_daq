...
*   \return  0=success; -1=error */
/* --------------------------------------------------------------------------------------------------------- */
// uint32_t * GetHistogram(int ch, uint32_t * EHisto)
// {
//     return EHisto;
// }

// I can save every second with this function
int SaveHistogram(char *basename, int b, int ch, uint32_t *EHisto)
{
	...
	*/
    ...
    sprintf(filename, "%s_%d_%d_%llu.txt", basename, b, ch, current_time);
    fh = fopen(filename, "w");
    if (fh == NULL)
		return -1;
    for(i=0; i<(1<<12); i++) {
    ...
    fclose(fh);
    // printf("Histograms saved to '%s_<board>_<channel>_<time>.txt'\n", basename);
