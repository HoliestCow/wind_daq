    ...
    *
    ...
    def registerPtu(self, unitDefinition):
        ...
        Parameters:
         - unitDefinition
        """
        self.registeredPTUs[unitDefinition.UUID] = unitDefinition
        newSessionId = generate_thrift_uuid()
        self.sessions[newSessionId] = {'unitDefinition': unitDefinition,
                                       'status': None,
    ...
    def define(self, sessionId, status, systemDefinition, systemConfiguration, recordingUpdate):
        ...
        if sessionId == self.current_sessionId:
            ...
            self.sessions[sessionId]['systemDefinition'] = systemDefinition
            self.sessions[sessionId]['systemConfiguration'] = systemConfiguration
            self.sessions[sessionId]['recordingUpdate'] = recordingUpdate
            # I don't think I supply this quite yet.
            # messageId = generate_thrift_uuid()
            # startRecordingPayload =
            # message = ControlMessage(commandId=messageId, type=ControlType.START_RECORDING,
...
if __name__ == '__main__':
    ################## CVRS STUFF ##################
    # handler = CVRSHandler()
    # handler._set_database('cvrs_db.sqlite3')
    # processor = Iface.Processor(handler)
